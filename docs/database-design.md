...
保存某门课对应教师的画像分析结果。该结果主要由 AI 根据课件、往年试题、实验要求、评分标准等资料自动归纳，字段包括出题风格、常见题型、评分偏好、重点章节、规避内容、分析状态、置信度和最近分析时间。用户可以对 AI 结果进行人工修正，也可以基于画像完整度、证据数量、证据平均置信度和来源多样性重新计算置信度。
