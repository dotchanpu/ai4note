...
AI 知识整理读取 `text_chunk` 中带页码的解析正文，通过 DeepSeek 输出结构化标签和知识条目。标签写入 `tag` 和 `material_tag`，知识条目写入 `knowledge_item`，并保留来源资料、章节、来源页码、知识类型与重要程度。自动关键词提取会先返回候选标签预览，用户确认后再写入 `material_tag`。

系统不使用简单关键词频率直接生成知识条目，而是要求模型基于全文语义合并重复概念，并区分定义、重点、公式、方法、例子和易错点。

## 相似资料检测

相似资料检测不新增持久化表。后端按课程范围读取 `material`、`material_tag`、`tag` 和 `text_chunk`，基于标题、摘要、标签交集和解析正文 token 计算相似度。上传后可以使用标题和摘要做初步检测，资料解析后会结合正文再次提示可能重复资料。
...
保存从往年真题中拆分出的题目，包括题号、题型、题干、答案、难度、分值、年份、来源页码等信息。当前还支持保存后续补全的参考答案来源说明和参考页码，用于标记答案来自哪份课件或笔记。
...
保存具体知识缺口条目，包括薄弱知识点、来源课程、关联课程关系、缺口类型、严重程度、原因和补学建议。前置课程知识缺口提示复用同一套判定规则，可直接返回未落库的提示列表；补学路径生成基于已保存的缺口条目实时分阶段组织，不新增表。

### teacher_profile

保存某门课对应教师的画像分析结果。该结果主要由 AI 根据课件、往年试题、实验要求、评分标准等资料自动归纳，字段包括出题风格、常见题型、评分偏好、重点章节、规避内容、分析状态、置信度和最近分析时间。用户可以对 AI 结果进行人工修正，也可以基于画像完整度、证据数量、证据平均置信度和来源多样性重新计算置信度；重新分析会覆盖同一画像记录并刷新证据来源。
...
保存 AI 生成任务记录。任务类型可以包括教师画像分析、真题知识点映射、知识缺口检测、资料摘要生成、资料关键词提取、复习提纲生成、模拟题生成、知识包摘要生成等。任务记录保存提示词、执行状态、结果路径和失败原因。模拟题生成任务使用 `MOCK_EXAM` 类型，并把 Markdown 结果文件的相对路径写入 `result_path`；按出题风格生成冲刺复习提纲时使用 `REVIEW_GENERATION` 类型，并将结果文件保存到 `sprint-outlines/` 相对路径。
