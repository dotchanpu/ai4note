...
保存 AI 生成任务记录。任务类型可以包括教师画像分析、真题知识点映射、知识缺口检测、资料摘要生成、资料关键词提取、复习提纲生成、模拟题生成、知识包摘要生成等。任务记录保存提示词、执行状态、结果路径和失败原因。模拟题生成任务使用 `MOCK_EXAM` 类型，并把 Markdown 结果文件的相对路径写入 `result_path`。
